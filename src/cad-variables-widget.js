...
class CadVariablesWidget extends HTMLElement {
  ...
  populateDropdown(selectEl, items) {
    // items may be strings or {label, value}
  ...
  async getVoiceTask() {
    ...
    return (
      tasks.find((t) => isTelephony(t) && isActive(t)) ||
  ...
  async refreshMeta() {
    try {
      const voiceTask = await this.getVoiceTask();
      if (!voiceTask) {
      ...
      const id =
        ...
        '(unknown)';
      this.$meta().textContent = `Interaction: ${id}`;
    } catch (err) {
  ...
  async handleSubmit(e) {
    ...
    try {
      ...
      // Safety: claim number is required (UI already enforces)
      if (!claim) {
      ...
      // Call Desktop SDK to update CAD variables on the current interaction
...
// As requested, the PGR_State dropdown uses the exact labels provided.
const STATES = [
...
// Long list of carriers. Keeping as strings so we save exact labels.
const CARRIERS = [
  "1st Auto & Casualty",
