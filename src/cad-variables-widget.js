...
class CadVariablesWidget extends HTMLElement {
  ...
  async refreshMeta() {
    try {
      ...
      // Also attempt prefill once, in case the initial call happened before connectedCallback fired
      this.prefillFromCad();
    } catch (err) {
  ...
  /**
   * NEW: Prefill Claim Number from CAD (PGR_ClaimNumber) if available.
  ...
  async prefillFromCad() {
    try {
      ...
      // ignore prefill errors—it's best-effort
    }
  }
  ...
  async handleSubmit(e) {
    ...
    try {
      ...
      // Your current behavior: send a single space for blank optional fields
      if (!state)    state = ' ';
      if (!carrier)  carrier = ' ';
